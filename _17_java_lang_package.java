...
public class _17_java_lang_package {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("String is immutable, whereas String BUffer is mutable and by default it's size is 16 characters. when u append more than this limit, tehn automaticalyl, enw string buffer of alrge size si created and the data is copied into it and new characters r added. String buffer is thread-safe i.e it only execute in synchronsied manner. No 2 threads can execute methods liek append(), insert() to the string buffer object at the same time. String buffer will not allow it. But, sicne it executes only synchronised, String buffer is little slow. SO, to overcome this issue, we use String Builder, which is mutable, fast than string buffer, but its not thread safe. i.e 2 threads can perform operations on the same stringbuilder object at the same time. SO, if ur sure, taht only one single thread is going to be ther or ur sure that no 2 threads would operate a same operation at a same time, u can choose String Builder. String overrides the Object's .equals() and .toHashCoder() method, whereas StringBuffer and StringBuilder won't. Because String is a value-based, immutable class. When you compare two strings, you're almost always interested in their characters/content, not whether they're the exact same object. So Java designers overrode .equals() in String to check content equality. StringBuffer / StringBuilder  doesn't  override .equals() Because these are mutable classes. So if .equals() were based on content, things would get weird. Mutable classes should not override .equals() unless they have a very good reason — to avoid breaking contracts in collections. That’s why StringBuffer and StringBuilder inherit default Object.equals() (which is reference-based). So, if u care about content comaprision for string buffer or stringbuilder, convert them to String adn thenc compare their content. The reason why String overrides hashCode() method is, By default, Java's Object.hashCode() method is based on memory address or internal identity, not content. Two objects with identical content but different memory locations will produce different hash codes by default. When you override equals() to compare content rather than identity (as String does), you must also override hashCode() to ensure that equal objects produce the same hash value. Note, if equals() returns true for two objects, their hashCode() methods must return the same value. However, two objects with the same hash code are not necessarily equal (this is called a hash collision).");
